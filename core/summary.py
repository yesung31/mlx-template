...
class ModelSummary:
    ...
    def _get_params(self, module):
        ...
        for _, p in mlx_utils.tree_flatten(params):
            n = p.size
            num_params += n
            # MLX doesn't have requires_grad flag on tensor yet easily accessible per-se
            # in same way, usually all are trainable if in optimizer
            trainable_params += n
            param_size += p.nbytes
    ...
    def summarize(self):
        ...
        # It's hard to separate root params from children params in simple recursive walk
        # if we just sum parameters()
        ...
        def walk(module, name_prefix="", depth=0):
            ...
            for name, child in module.children().items():
                ...
                if isinstance(child, nn.Module):
                    ...
                    # If we want nested. PL usually just shows top level or flattened.
        ...
        # Summary footer
        # Re-calculate total exact from full model to be sure
        # (in case of shared params or root params)
